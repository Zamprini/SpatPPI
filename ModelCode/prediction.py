...
def main(data_folder, model_path, predicted_filename):
    ...
    # Set paths
    test_path = os.path.join(data_folder, predicted_filename)
    ...
    if torch.cuda.is_available():
        model.load_state_dict(torch.load(model_path))  # Load model to GPU if available
    else:
    ...
    trainer = Trainer(model, None, device, test_generator, test_generator, test_generator, opt_da=None, discriminator=None, experiment=None, **cfg)
...
if __name__ == "__main__":
    # Argument parser
    parser = argparse.ArgumentParser(description="Train and Test PPI Model")
    ...
    parser.add_argument('--predicted_filename', default="Example_Test.csv", type=str, help='The filename of the test dataset under data_folder(csv file')

    # Parse arguments
    args = parser.parse_args()

    # Run main function
    main(args.data_folder, args.model_path, args.predicted_filename)
